...
sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/secure/server.local.key -out /etc/secure/server.local.crt
...
sudo sshpass -p "vagrant" scp -o StrictHostKeyChecking=no /etc/secure/server.local.crt vagrant@192.168.1.200:~/server.local.crt